/**
 * For this part for drawing the map we refered to this website
 * https://bl.ocks.org/dnprock/5215cc464cfb9affd283
 * But I changed everything to fit  it for our own dataset.
 *
 */


 var drawMap = function (investor, path) {
...
//defining the min and max color
        var config = {
                "color1" : "#f5e9e9",
                "color2": "#b00e13",
        ...
        var fundingById = d3version3.map(),rankById = d3version3.map(),numById = d3version3.map(), numById2= d3version3.map();
        ...
        var colors = [];
            ...
            //mapping data to corresponding color
        ...
        var svg = d3version3.select("#canvas-svg").append("svg")
            .attr("width", width)
            .attr("height", height)


        //start drawing the map, we first get the states names and their codes
        d3version3.tsv("https://s3-us-west-2.amazonaws.com/vida-public/geo/us-state-names.tsv", function (error, names) {
            ...
            stateList.forEach(function (d) {
                ...
                if (investor=="...All Investors..."){
                    numById.set(id, +d[MAP_NUM]/d3version3.max(stateList, function (d) {
                        return +d[MAP_NUM]
                    })*1000);
                ...
                numById2.set(id, +d[MAP_NUM])
            });
            // here we start drawing each state and color it
            d3version3.json("https://s3-us-west-2.amazonaws.com/vida-public/geo/us.json", function (error, us) {
                svg.append("g")
                    ...
                    .attr("d", path)
                    .on("mousemove", function (d) {  // we add interaction for hovering over tha map and showing information
                        ...
                        if (numById.get(d.id)) {
                            ...
                            html += "<span class=\"tooltip_key\">" + "Number of Companies" + "</span>" + "<span class=\"tooltip_value\">" + (numById2.get(d.id) ? valueFormat(numById2.get(d.id)) : "") + "</span>" + "<br/>"
                            html += "</span>";
...
// defining fuctions for color conversion and formating of large numbers
